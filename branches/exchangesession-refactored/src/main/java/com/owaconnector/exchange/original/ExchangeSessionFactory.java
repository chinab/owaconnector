...
import com.owaconnector.exception.NetworkDownException;
...
public final class ExchangeSessionFactory {
	...
	static class PoolKey {
		...
		public boolean equals(Object object) {
			return object == this || object instanceof PoolKey
	 ...
	 */
	public static ExchangeSession getInstance(URI url, String userName,
			String password) throws IOException, DavMailException {
		ExchangeSession session = null;
		try {
			PoolKey poolKey = new PoolKey(url.toURL().toExternalForm(),
					userName, password);
			...
			if (session != null) {
				ExchangeSession.LOGGER.debug("Got session " + session
						+ " from cache");
			}
			...
			if (session == null) {
				session = new ExchangeSession(poolKey.url, poolKey.userName,
						poolKey.password);
	...
	/**
	 * Get a non expired session. If the current session is not expired, return
	 * current session, else try to create a new session
	 * 
	 ...
	 * @throws DavMailException
	 */
	public static ExchangeSession getInstance(ExchangeSession currentSession,
			URI url, String userName, String password) throws IOException,
			DavMailException {
	...
	public static void setConfigChecked(boolean configChecked) {
		ExchangeSessionFactory.configChecked = configChecked;
	}
