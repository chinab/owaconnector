...
public class BundleMessage implements Serializable {
	...
	/**
	 * Format message for logging (with the root locale). Log file should remain
	 * in english
	 * 
	...
	/**
	 * Get bundle for the given locale. Load the properties file for the given
	 * locale in a resource bundle
	 * 
	 * @param locale
	 *            resource bundle locale
	...
	public static String format(Locale locale, String key, Object... arguments) {
		...
		if (arguments != null) {
			...
			for (int i = 0; i < arguments.length; i++) {
				if (arguments[i] instanceof BundleMessage) {
					formattedArguments[i] = ((BundleMessage) arguments[i])
							.format(locale);
					...
					formattedArguments[i] = ((DavMailException) arguments[i])
							.getMessage(locale);
				} else if (arguments[i] instanceof Throwable) {
					formattedArguments[i] = ((Throwable) arguments[i])
							.getMessage();
		...
		return MessageFormat.format(getBundle(locale).getString(key),
				formattedArguments);
	}
	 ...
	 * @return formatted message
	 */
	public static String getExceptionLogMessage(BundleMessage message,
			Exception e) {
		return getExceptionMessage(message, e, ROOT_LOCALE);
	}

	/**
	 * Get formatted error message for bundle message and exception with default
	 * locale.
	 * 
	 * @param message
	 *            bundle message
	...
	/**
	 * Get formatted error message for bundle message and exception with given
	 * locale.
	 * 
	 * @param message
	 *            bundle message
	 ...
	 * @return formatted message
	 */
	public static String getExceptionMessage(BundleMessage message,
