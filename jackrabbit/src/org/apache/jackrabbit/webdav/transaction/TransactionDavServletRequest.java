...
public interface TransactionDavServletRequest extends DavServletRequest {
	 ...
	 * 
	 * @return transaction id as present in the
	 *         {@link TransactionConstants#HEADER_TRANSACTIONID TransactionId
	 ...
	 * provide the information required (either because it is missing or the Xml
	 * is not valid) <code>null</code> is returned.
	 * 
	 * @return <code>TransactionInfo</code> object encapsulating the
	 *         'transactioninfo' Xml element present in the request body or
	 *         <code>null</null> if no
     * body is present or if it could not be parsed.
	 * @throws DavException
	 *             if an invalid request body is present.
	 */
