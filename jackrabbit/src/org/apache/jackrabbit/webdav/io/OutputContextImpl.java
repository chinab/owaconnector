...
public class OutputContextImpl implements OutputContext {
	...
	public void setContentLanguage(String contentLanguage) {
		if (contentLanguage != null) {
			response.setHeader(DavConstants.HEADER_CONTENT_LANGUAGE,
					contentLanguage);
		}
	}
	...
	public void setContentLength(long contentLength) {
		if (contentLength >= 0) {
			if (contentLength <= Integer.MAX_VALUE) {
				...
				response.addHeader(DavConstants.HEADER_CONTENT_LENGTH, Long
	...
	public void setModificationTime(long modificationTime) {
		if (modificationTime >= 0) {
			response.addDateHeader(DavConstants.HEADER_LAST_MODIFIED,
					modificationTime);
		}
	}
