...
public class CopyMethod extends DavMethodBase {
	...
	public CopyMethod(String uri, String destinationUri, boolean overwrite,
			boolean shallow) {
		super(uri);
		addHeader(DavConstants.HEADER_DESTINATION, destinationUri);
		addHeader(new OverwriteHeader(overwrite));
		if (shallow) {
			addHeader(new DepthHeader(false));
		}
	}
	 ...
	 *         (Created)} or {@link DavServletResponse#SC_NO_CONTENT 204 (No
	 *         Content)}.
	 */
