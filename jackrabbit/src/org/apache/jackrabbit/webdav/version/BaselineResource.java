 ...
 * Such as new versions are created by CHECKIN of a version-controlled resource,
 * a new baseline is created, whenever the VCC resource, that represents a set
 * of resources rather than a single resource, is checked-in.
 * <p/>
...
public interface BaselineResource extends VersionResource {
	 ...
	 * collection that lists as members all version-controlled resources of the
	 * configuration this baseline belongs to (the baseline being one version of
	 * the corresponding vc-configuration-resource). In other words: each member
	 * in the list must correspond to a member of the baseline-controlled
	 ...
	 */
	public static final DavPropertyName BASELINE_COLLECTION = DavPropertyName
			.create("baseline-collection", DeltaVConstants.NAMESPACE);

	/**
	 * The protected DAV:subbaseline-set property identifies a set of baseline
	 * resources. Note however, that the subbaselines of this resource are not
	 * only formed from the baseline resources listed in this property but also
	 * includes all subbaseline resources of the latter.
	 * 
	 ...
	 * {@link #BASELINE_COLLECTION} property.
	 * 
