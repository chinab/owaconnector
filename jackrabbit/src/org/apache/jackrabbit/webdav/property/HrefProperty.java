...
public class HrefProperty extends AbstractDavProperty {
	 ...
	 * object, consisting of {@link #XML_HREF href} <code>Element</code>
	 * entries.
	 * 
	 * @param prop
	 */
			...
			if (val instanceof List<?>) {
				...
				while (it.hasNext()) {
					...
					if (o instanceof Element
							&& XML_HREF.equals(((Element) o).getLocalName())) {
						String href = DomUtil.getText((Element) o);
						if (href != null) {
							hrefList.add(href);
						} else {
							log
									.warn("Valid DAV:href element expected instead of "
											+ o.toString());
						}
					} else {
						log
								.warn("DAV: href element expected in the content of "
										+ getName().toString());
					}
				}
			} else if (val instanceof Element
					&& XML_HREF.equals(((Element) val).getLocalName())) {
				String href = DomUtil.getTextTrim((Element) val);
				if (href != null) {
					hrefList.add(href);
				} else {
					log.warn("Valid DAV:href element expected instead of "
							+ val.toString());
				}
			}
	 ...
	 *            A value of true, defines this property to be invisible in
	 *            PROPFIND/allprop It will not be returned in a
	 *            {@link org.apache.jackrabbit.webdav.DavConstants#PROPFIND_ALL_PROP
	 *            DAV:allprop} PROPFIND request.
	 */
	public HrefProperty(DavPropertyName name, String value,
	...
	public List<?> getHrefs() {
		return (value != null) ? Arrays.asList(value) : new ArrayList<Object>();
	}
