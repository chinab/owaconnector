...
public class ExchangeSession {
     ...
     */
    ExchangeSession(String url, String userName, String password) throws IOException, HttpException, URISyntaxException {
        ...
        try {
            ...
            HttpResponse response = DavGatewayHttpClientFacade.executeFollowRedirects(httpClient, method);
            ...
            if (isBasicAuthentication) {
                ...
                if (status == HttpStatus.SC_UNAUTHORIZED) {
                	...
                	//response.abort();
                    throw DavGatewayHttpClientFacade.buildHttpException(null, response);
                }
            } else {
                response = formLogin(httpClient, method, response,userName, password);
            }
     ...
     * @throws HttpException 
     */
    public boolean isExpired() throws NoRouteToHostException, UnknownHostException, HttpException {
    ...
    protected String getAbsoluteUri(HttpUriRequest method, String path) throws URISyntaxException {
        URI uri = method.getURI();
    ...
    protected String getScriptBasedFormURL(HttpUriRequest initmethod, String pathQuery) throws URISyntaxException {
     ...
     * @param initresponse form body http method
     * @return logon method
     * @throws IOException on error
     * @throws URISyntaxException 
     */
    protected HttpPost buildLogonMethod(DefaultHttpClient httpClient, HttpUriRequest initmethod, HttpResponse initresponse) throws IOException, URISyntaxException {

        HttpPost logonMethod = null;

        // create an instance of HtmlCleaner
        HtmlCleaner cleaner = new HtmlCleaner();
     //   String html = EntityUtils.toString(initresponse.getEntity());
        try {
            ...
            if (forms.size() == 1) {
                ...
                logonMethod = new HttpPost(getAbsoluteUri(initmethod, logonMethodPath));
                ...
                for (Object input : inputList) {
                    ...
                    if (USER_NAME_FIELDS.contains(name)) {
                        ...
                        HttpResponse newInitResponse = DavGatewayHttpClientFacade.executeFollowRedirects(httpClient, logonMethod);
                        logonMethod = buildLogonMethod(httpClient, logonMethod, newInitResponse);
                ...
                if (frameList.size() == 1) {
                    ...
                    if (src != null) {
                      ...
                      //  initresponse.abort();
                		...
                		HttpClientParams.setRedirecting(newInitMethod.getParams(), true);
                        HttpResponse newInitResponse = DavGatewayHttpClientFacade.executeFollowRedirects(httpClient, newInitMethod);
                        logonMethod = buildLogonMethod(httpClient, newInitMethod,newInitResponse);
                    }
                } else {
                	// FIXME
                        ...
                        for (Object content : contents) {
                            if (content instanceof CommentToken) {
                                ...
                                if (sUrl != null && sLgn != null) {
                                    ...
                                    HttpRequestBase newInitMethod = new HttpGet(src);
                            		HttpClientParams.setRedirecting(newInitMethod.getParams(), false);
                                    HttpResponse response = DavGatewayHttpClientFacade.executeFollowRedirects(httpClient, newInitMethod);
...
//        } finally {
//          //  initresponse.releaseConnection();
        }

        return logonMethod;
    }

    protected HttpResponse formLogin(DefaultHttpClient httpClient, HttpUriRequest initmethod, HttpResponse initresponse,String userName, String password) throws IOException, DavMailException {
       ...
       // logonMethod.setEntity(entity);
        ...
        HttpResponse response = DavGatewayHttpClientFacade.executeFollowRedirects(httpClient, logonMethod);
		///logonMethod = response;
    ...
    protected void checkFormLoginQueryString(HttpPost logonMethod) throws DavMailAuthenticationException {
    ...
    protected void buildMailPath(HttpUriRequest method,HttpResponse response) throws HttpException {
        // find base url
        String line;
        ...
        try {
            mainPageReader = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
    ...
    protected String getURIPropertyIfExists(DavPropertySet properties, String name, Namespace namespace) throws URISyntaxException {
        DavProperty property = properties.get(name, namespace);
        if (property == null) {
            return null;
        } else {
    ...
    protected void getWellKnownFolders() throws HttpException, URISyntaxException {
        // Retrieve well known URLs
        MultiStatusResponse[] responses = null;
        try {
            responses = DavGatewayHttpClientFacade.executePropFindMethod(
                    httpClient, getBaseUrl() + URIUtil.encodePath(mailPath), 0, WELL_KNOWN_FOLDERS);
...
//    }
     ...
     * @throws HttpException 
     */
    public List<Folder> getSubFolders(String folderName, boolean recursive) throws IOException, HttpException {
     ...
     * @throws HttpException 
     */
    public List<Folder> getSubCalendarFolders(String folderName, boolean recursive) throws IOException, HttpException {
     ...
     * @throws HttpException 
     */
    public List<Folder> getSubFolders(String folderName, String filter, boolean recursive) throws IOException, HttpException {
    ...
    protected Folder buildFolder(MultiStatusResponse entity) throws IOException, DavMailException {
...
//    }
     ...
     * @throws HttpException 
     */
    public Folder getFolder(String folderName) throws IOException, HttpException {
     ...
     */
    @Deprecated
//    public Folder refreshFolder(Folder currentFolder) throws IOException, HttpException {
...
//    }
     ...
     * @throws HttpException 
     */
    public void createMessageFolder(String folderName) throws IOException, HttpException {
     ...
     * @throws HttpException 
     */
    public void createCalendarFolder(String folderName) throws IOException, HttpException {
     ...
     * @throws HttpException 
     */
    public void createFolder(String folderName, String folderClass) throws IOException, HttpException {
        ...
        HttpResponse response = DavGatewayHttpClientFacade.executeHttpMethod(httpClient, method);
		int status = response.getStatusLine().getStatusCode();
        // ok or alredy exists
        if (status != HttpStatus.SC_MULTI_STATUS && status != HttpStatus.SC_METHOD_NOT_ALLOWED) {
            throw DavGatewayHttpClientFacade.buildHttpException(method, response);
        }
    }
...
//    }
     ...
     * @throws IOException on error
     */
    public void moveFolder(String folderName, String targetName) throws HttpException {
        String folderPath = getFolderPath(folderName);
...
//    protected void moveToTrash(String encodedMessageUrl) throws IOException, HttpException {
...
//        }
    ...
    public class Message implements Comparable<Message> {
        ...
        public void write(OutputStream os) throws IOException, DavMailException {
            ...
            try {
                HttpResponse response = DavGatewayHttpClientFacade.executeGetMethod(httpClient, method, false);
         ...
         * @throws DavMailException 
         */
        public MimeMessage getMimeMessage() throws IOException, MessagingException, DavMailException {
            if (mimeMessage == null) {
    ...
    public class Event {
        ...
        public String getICS() throws IOException, HttpException {
            ...
            try {
                ...
                MimeMessage mimeMessage = new MimeMessage(null, response.getEntity().getContent());
        ...
        protected String getAllDayLine(String line) throws IOException,HttpException {
        ...
        protected String fixICS(String icsBody, boolean fromServer) throws IOException, HttpException {
            // first pass : detect
            class AllDayState {
                boolean isAllDay;
...
//            int dumpMax = 1000; //Settings.getIntProperty("davmail.dumpICS");
//            if (dumpMax > 0) {
...
//            putmethod.addHeader("Translate", "f");
//            putmethod.addHeader("Overwrite", "f");
//            if (etag != null) {
//                putmethod.addHeader("If-Match", etag);
//            }
//            if (noneMatch != null) {
//                putmethod.addHeader("If-None-Match", noneMatch);
//            }
//            putmethod.addHeader("Content-Type", "message/rfc822");
...
//            try {
//                if (status == 0) {
//                    response = httpClient.execute(putmethod.getRequest());
...
//                        LOGGER.warn("Unable to create or update message " + status + ' ' + response.getStatusLine());
//                    }
//                }
//            } finally {
//                putmethod.releaseConnection();
...
//            if (response.getFirstHeader("GetETag") != null) {
//                eventResult.etag = response.getFirstHeader("GetETag").getValue();
//            }
//
...
//    }
     ...
     * @throws HttpException 
     */
    public List<Event> getEventMessages(String folderPath) throws IOException, URISyntaxException, HttpException {
        List<Event> result;
     ...
     * @throws HttpException 
     */
    public List<Event> getAllEvents(String folderPath) throws IOException, URISyntaxException, HttpException {
        int caldavPastDelay = 100;// Settings.getIntProperty("davmail.caldavPastDelay", Integer.MAX_VALUE);
     ...
     * @throws HttpException 
     */
    protected List<Event> getEvents(String folderPath, String searchQuery) throws IOException, URISyntaxException, HttpException {
        List<Event> events = new ArrayList<Event>();
        MultiStatusResponse[] responses = DavGatewayHttpClientFacade.executeSearchMethod(httpClient, getBaseUrl()+ URIUtil.encodePath(folderPath), searchQuery);
        for (MultiStatusResponse response : responses) {
     ...
     * @throws HttpException 
     */
    public Event getEvent(String folderPath, String eventName) throws IOException, URISyntaxException, HttpException {
     ...
     * @throws HttpException 
     */
    public Event getEvent(String eventPath) throws IOException, URISyntaxException, HttpException {
     ...
     * @throws HttpException 
     */
    public int deleteEvent(String folderPath, String eventName) throws IOException, HttpException {
    ...
    protected Event buildEvent(MultiStatusResponse calendarResponse) throws URISyntaxException {
     ...
     * @throws HttpException 
     */
//    public int sendEvent(String icsBody) throws IOException, HttpException {
...
//    }
     ...
     * @throws HttpException 
     */
    public String getFolderResourceTag(String folderPath) throws IOException, HttpException {
        return getFolderProperty(folderPath, RESOURCE_TAG);
    }

    protected String getFolderProperty(String folderPath, DavPropertyNameSet davPropertyNameSet) throws IOException, HttpException {
        String result;
        MultiStatusResponse[] responses = DavGatewayHttpClientFacade.executePropFindMethod(
     ...
     * @throws DavMailException 
     */
    public String buildCalendarPath(String principal, String folderName) throws IOException, DavMailException {
        StringBuilder buffer = new StringBuilder();
    ...
    public String getEmail(String alias) throws DavMailException {
        ...
        if (alias != null) {
            ...
            String path = null;
            try {
                path = getBaseUrl() + getCmdBasePath() + "?Cmd=galfind&AN=" + URIUtil.encodePath(alias);
                getMethod = new HttpGet(path);
                HttpResponse response = DavGatewayHttpClientFacade.executeGetMethod(httpClient, getMethod, true);
                Map<String, Map<String, String>> results = XMLStreamUtil.getElementContentsAsMap(response.getEntity().getContent(), "item", "AN");
...
//                }
     ...
     * @throws HttpException 
     */
    public void buildEmail(String hostName, String methodPath) throws HttpException {
    ...
    protected String getAliasFromOptions(String path) throws DavMailException {
        ...
        try {
            HttpResponse response = DavGatewayHttpClientFacade.executeGetMethod(httpClient, optionsMethod, false);
            optionsPageReader = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
            String line;
            ...
            LOGGER.error("Error parsing options page at " + optionsMethod.getURI().getPath());
        } finally {
    ...
    public Map<String, Map<String, String>> galFind(String searchAttribute, String searchValue) throws IOException {
        ...
        HttpGet getMethod = new HttpGet(URIUtil.encodePath(getCmdBasePath() + "?Cmd=galfind&" + searchAttribute + '=' + searchValue));
            HttpResponse response;
			try {
				response = DavGatewayHttpClientFacade.executeGetMethod(httpClient, getMethod, true);
...
//    }
    ...
    public void galLookup(Map<String, String> person) throws DavMailException {
        if (!disableGalLookup) {
            HttpGet getMethod = null;
            try {
                getMethod = new HttpGet(URIUtil.encodePath(getCmdBasePath() + "?Cmd=gallookup&ADDR=" + person.get("EM")));
                HttpResponse response = DavGatewayHttpClientFacade.executeGetMethod(httpClient, getMethod, true);
     ...
     * @throws DavMailException 
     */
    public FreeBusy getFreebusy(String attendee, String startDateValue, String endDateValue) throws IOException, DavMailException {
        ...
        try {
            ...
            String fbdata = StringUtil.getLastToken(EntityUtils.toString(response.getEntity()), "<a:fbdata>", "</a:fbdata>");
            if (fbdata != null) {
    ...
    protected final class VTimezone {
        ...
        private void load() throws HttpException {
            try {
                ...
                try {
                    // create fake event
                    HttpResponse response = ExchangeSession.this.httpClient.execute(postMethod);
					int statusCode = response.getStatusLine().getStatusCode();
                    if (statusCode == HttpStatus.SC_OK) {
                        fakeEventUrl = StringUtil.getToken(EntityUtils.toString(response.getEntity()), "<span id=\"itemHREF\">", "</span>");
                    }
                } finally {
                    postMethod.abort();
                }
                ...
                if (fakeEventUrl != null) {
                    // get fake event body
                	HttpGet getMethod = new HttpGet(URIUtil.encodePath(fakeEventUrl));
                    getMethod.addHeader("Translate", "f");
                    try {
                        HttpResponse response = ExchangeSession.this.httpClient.execute(getMethod);
                        timezoneBody = "BEGIN:VTIMEZONE" +
                                StringUtil.getToken(EntityUtils.toString(response.getEntity()), "BEGIN:VTIMEZONE", "END:VTIMEZONE") +
                                "END:VTIMEZONE\r\n";
